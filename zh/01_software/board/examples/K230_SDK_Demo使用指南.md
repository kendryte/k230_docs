...
当前版本支持OV9732和OV9286、imx335 三个摄像头模组图像采集预览，支持单个摄像头最多输出三路数据流。
...
![图片描述](images/3e2e061af6118ae112633da2aaba6376.png)
...
说明：将ov9732@1280x720 RGB 绑定到vicap设备,并使能vicap设备输出通道0和通道1，其中通道0输出大小默认为输入图像大小（1280x720），通道1输出图像大小为640x480
...
参考 [K230_USB应用实战_UVC传输YUV及编码码流](../../../02_reference/software/K230_USB%E5%BA%94%E7%94%A8%E5%AE%9E%E6%88%98_UVC%E4%BC%A0%E8%BE%93YUV%E5%8F%8A%E7%BC%96%E7%A0%81%E7%A0%81%E6%B5%81.md)
...
默认使用BULK传输，使用以下命令可以更改为ISO传输。但是当前的USB驱动采用buffer DMA方式，ISO传输的性能比较低，需要改成desc DMA方式。
...
作为host，连接U盘，连接鼠标键盘。需要使用跳线帽连接J5的pin1/pin2，使能5V供电。
...
连接开发板的USB1，typeC连接PC，PC上显示U盘连接。容量为1.98MB。

`gadget-storage-mem.sh` 支持把SD/eMMC的FAT分区当作U盘的分区使用。
...
K230开发板USB0通过typeC转typeA连接鼠标或键盘。

```shell
[root@canaan / ]$ test_mouse /dev/input/event0
#点击或移动鼠标，串口会有对应的显示。

[root@canaan / ]$ test_keyboard /dev/input/event0
#按下键盘不同的按键，串口会有对应的显示。
```
...
![卡通人物 描述已自动生成](images/e54764629a9e9fb63ec4340d316e4f42.png)

##### 2.11.4.2 linearGrad

运行`linearGrad`命令，执行完成后在当前目录生成linearGrad.png，如下图所示

![图片描述](images/24db1c231608cd26a5a981e055aa6910.png)
...
![图片描述](images/9f7f9bfaedf0f183b63b59ace6e1569e.png)

- `imgIndex2.png`: index2模式，支持4种颜色

![图片描述](images/c4bb92a96fd0ba1f4973a27ecd6f60eb.png)

- `imgIndex4.png`: index4模式，支持16种颜色

![图片描述](images/9a3c47da0975b0226e75453a79a8da2c.png)

- `imgIndex8.png`: index8模式，支持256种颜色

![背景图案 描述已自动生成](images/1f24a6530d8c0d1ed22bb3eea26a33fb.png)
...
1. 通过 ` lsmod ` 检查小核侧是否加载k_ipcm模块，如未加载，执行 `insmod k_ipcm.ko` 加载k_ipcm模块
1. 在大核侧启动核间通信进程，执行 `./sample_sys_inif.elf`
1. 在小核侧/mnt目录下，执行 ` ./rtsp_demo `，默认为1路h265视频编码推流，分辨率为1280x720，如需传参参看如下参数说明：
...
可通过` ctrl+c `停止运行，小核上正常运行rtsp_demo后，会打印出形如：`rtsp://ip:8554/session0` 的url地址，其中 0 代表第0路，可通过vlc拉取url的流进行播放。
